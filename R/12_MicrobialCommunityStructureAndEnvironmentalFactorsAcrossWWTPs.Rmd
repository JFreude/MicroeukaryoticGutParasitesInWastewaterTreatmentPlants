---
title: "Microbial community structure and environmental factors across WWTPs"  
...
NMDS biplots show the microbial community composition across WWTP compartments and locations, in association with environmental data. Metagenomic and metatranscriptomic data are shown separately. Samples are color-coded and grouped (ellipses) by compartment. Significant environmental vectors are shown as arrows (* p < 0.05; ** p < 0.01; *** p < 0.001). Environmental vectors: DO = dissolved oxygen, DOC = dissolved organic carbon, HRT = Hydraulic retention time, TN = total nitrogen, TP = total phosphorus.

This analysis shows that environmental factors such as pH, total phosphorus and dissolved organic matter correlated significantly with the diversity of the microbial community at the WWTP level.
...
# Save plot as pdf
pdf("./Results/NMDS/NMDS_AcrossWWTPs.pdf", width = 10, height = 4.15)
print(NMDS.plots)
dev.off()
```