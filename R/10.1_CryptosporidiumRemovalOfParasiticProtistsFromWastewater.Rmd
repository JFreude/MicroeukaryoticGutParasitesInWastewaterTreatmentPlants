...
parasitic_protists <- c("Acanthamoebidae_X", "Blastocystis", "Copromyxa", "Cryptosporidium",
                        "Dientamoeba", "Entamoeba", "Giardia", "Guttulinopsis", "Rosculus")
...
for(data_type in c("DNA", "RNA")){
  ...
  # Replace all Acanthamoebidae genera with Acanthamoebidae_X
  taxonomy[taxonomy$Family == "Acanthamoebidae", "Genus"] <- "Acanthamoebidae_X"
  ...
  # Sum by parasites
  taxonomy$Parasites <- taxonomy$Genus
  taxonomy[taxonomy$Parasites == "Acanthamoebidae_X", "Parasites"] <- "Acanthamoebidae"
  normalized_counts <- cbind(Parasites = taxonomy$Parasites, normalized_counts)
  normalized_counts <- data.frame(normalized_counts %>% 
                                    group_by(Parasites) %>%
                                    summarise_if(is.numeric, sum) %>% 
                                    mutate_if(is.factor, as.character)) 
  rownames(normalized_counts) <- paste(seq(nrow(normalized_counts)), normalized_counts$Parasites, sep = "_")
  taxonomy <- data.frame(normalized_counts %>% select_if(is.character))
  normalized_counts <- data.frame(normalized_counts %>% select_if(is.numeric))
  
  ## Boxplots
  # All column specifying the genera to normalized abundance data 
  data_boxplot <- cbind(Parasites=taxonomy$Parasites, normalized_counts)
  
  # Reshape data frames, on column per 'Parasites', 'Sample ID' and 'normalized abundance'
  data_boxplot <- melt(data_boxplot, id=(c("Parasites")))
...
Sign_tests <- setNames(data.frame(matrix(ncol = 5, nrow = 0)), c("Data", "Parasites", 
                                                                 "Group1", "Group2", "p"))

# Split data frame to one list per data type and Genus
data_Sign_test <-  lapply(data_sign_test %>% group_split(Data, Parasites), data.frame)
...
for(data_set in data_Sign_test){
  ...
  Sign_tests <- rbind(Sign_tests, data.frame(Data = unique(INF_DNF$Data), 
                                             Parasites = unique(INF_DNF$Parasites), 
  ...
  Sign_tests <- rbind(Sign_tests, data.frame(Data = unique(NFC_EFF$Data),
                                             Parasites = unique(NFC_EFF$Parasites), 
                                             Group1 = sign_test$group1, 
                                             Group2 = sign_test$group2, p = sign_test$p))
}

# Order table
Sign_tests <- Sign_tests[order(Sign_tests$Group1, Sign_tests$Data, Sign_tests$Parasites),]
Sign_tests <- data.frame(Sign_tests %>% mutate_if(is.factor, as.character))

# Genus names italic
Sign_tests[Sign_tests$Parasites != "Acanthamoebidae","Parasites"]  <- 
  cell_spec(Sign_tests[Sign_tests$Parasites != "Acanthamoebidae","Parasites"],italic = T) 
...
colors <- c("#D73027", "#FDAE61", "#FEE090", "#B8E186", "#1B7837", "#4575B4", "#ABD9E9", "mediumpurple", "plum3")
...
for(data_type in names(data_lineplots)){
  ...
  if(data_type == names(data_lineplots)[2]){
    scaling <- max( plot_data[ plot_data$Parasites != "Rosculus", "Mean"] + 
                      plot_data[ plot_data$Parasites != "Rosculus", "Standard Deviation"])/
      max( plot_data[ plot_data$Parasites == "Rosculus", "Mean"] +
             plot_data[ plot_data$Parasites == "Rosculus", "Standard Deviation"])
     plot_data[ plot_data$Parasites == "Rosculus", "Mean"] <-  
       plot_data[ plot_data$Parasites == "Rosculus", "Mean"]*scaling
     plot_data[ plot_data$Parasites == "Rosculus", "Standard Deviation"] <-  
       plot_data[ plot_data$Parasites == "Rosculus", "Standard Deviation"]*scaling
  } 
  
  ########
  scaling <- max( plot_data[ plot_data$Parasites != "Acanthamoebidae", "Mean"] + 
                      plot_data[ plot_data$Parasites != "Acanthamoebidae", "Standard Deviation"])/
      max( plot_data[ plot_data$Parasites == "Acanthamoebidae", "Mean"] +
             plot_data[ plot_data$Parasites == "Acanthamoebidae", "Standard Deviation"])
     
  plot_data[ plot_data$Parasites == "Acanthamoebidae", "Mean"] <-  
       plot_data[ plot_data$Parasites == "Acanthamoebidae", "Mean"]*scaling
  
  plot_data[ plot_data$Parasites == "Acanthamoebidae", "Standard Deviation"] <-  
       plot_data[ plot_data$Parasites == "Acanthamoebidae", "Standard Deviation"]*scaling
  ########
  
  # Assign colors to taxa
  plot_data <- merge(plot_data, data.frame(Parasites=unique( plot_data$Parasites), Colors=colors), 
                     all = T)

  # Basic plot 
  plot <- line.area.plot(plot_data, plot.type="line.smooth", x.axis="X", 
                         y.axis="Mean", fill.order="Fill", sd="Standard Deviation", 
                         legend.names = "Parasites", color = "Colors", negative.sd=F) +theme_classic()
  
  # Adjust plot
  if(data_type == names(data_lineplots)[1]){
    plot <- plot + 
      scale_y_continuous(sec.axis = sec_axis(~./scaling, name = expression(atop("Acanthamoebidae",#italic("Acanthamoebidae"), 
                                                                                "Relative abundances [%]"))))+
    ...
    plot <- plot + 
      scale_y_continuous(sec.axis = sec_axis(~./scaling, name = expression(atop("Acanthamoebidae",#italic("Rosculus"), 
                                                                                "Relative abundances [%]"))))+
      theme(axis.title.y.right = element_text(vjust = 1.5))
  ...
  plot <- style.sheet(plot, plot.title = data_type, x.axis.text.labels = 
                      ...
                      x.axis.label = "", show.legend.by.fill = FALSE, legend.text.style = "italic")
...
# Save plot as pdf
pdf("./Results/Apicomplexa/LineplotsParasitesAcanthamoebidae_noText2.pdf", width = 10, height = 3.25)
print(lineplots)
dev.off()
```
